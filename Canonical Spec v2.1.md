...
- **Proved results registry:** Now **46 Category A**, **1 Category B**, **1 Category C**, **0 retracted**; total 48 claims (Phase 12: T-Persist-1(b) upgraded Cat B → A via Theorem BC' + PSM).
...
The proto-cohesion diagnostic vector $\mathbf{d} \in [0,1]^4$ unifies these four requirements into a graded formal assessment of formation quality, and the minimal energy principle — with mandatory volume constraint on the manifold $\Sigma_m$ — provides a variational characterization of formations. The theory has **46 fully proved theorems** (Category A), 1 with structural parameter (Category B), and 1 conditional result (Category C), with 0 retracted claims — totaling **48 formal claims, 95.8% fully proved** (Phase 12 upgrade: T-Persist-1(b) basin containment Cat A via Theorem BC' + Theorem PSM; Kupka-Smale + Sard genericity removes (NB) hard threshold). Key results include non-trivial minimizer existence under a computable phase transition (T8-Core, T8-Full), gradient flow convergence (T14), stability advantage for non-idempotent closure (T3/T6), exact predicate-energy bridge (Sep = 1 - E_sep/m), and the unified multi-formation persistence theorem (T-Persist-K-Unified) parametrized by the coupling measure $\Lambda_{\mathrm{coupling}}$. Phase 12 upgrades: T-Persist-1(b) basin containment (directional basin radius via BC', soft-mode fraction via PSM, unconditional Kupka-Smale/Sard arguments), Spec consistency restored (lines 1010-1011, 1084 corrected). Core persistence chain (all 5 parts T-Persist-1(a-e)) now 100% Category A.
...
The theory's largest remaining gaps (Phase 12 completion: 95.8% Cat A, after T-Persist-1(b) basin containment Cat A upgrade) are: (1) general-graph FORMATION-BIRTH (proved for D₄-symmetric only, Cat A; general case Cat C); (2) near-bifurcation persistence (μ → 0, branch selection, center manifold reduction, Cat C); (3) T-Bind-Proj general τ case (proved for τ = 1/2, Cat B; general τ requires binary-approximation gap analysis). The unified regime parametrization ($\Lambda_{\mathrm{coupling}}$) provides a continuous bridge between well-separated, weakly-interacting, and strongly-interacting regimes with 100% experimental validation (exp46-47). T-Persist-K-Unified covers persistence below the merge bifurcation ($\Lambda < 1/(K{-}1)$); above this threshold, kinetic barrier-crossing dynamics dominate and the merge problem begins. Multi-formation temporal persistence is fully or conditionally resolved for: well-separated regime (T-Persist-K-Sep, **proved**), weakly-interacting regime (T-Persist-K-Weak, conditionally proved), parametric strong-coexistence regime below merge (T-Persist-K-Unified Corollary III, conditional), and basin containment/transport confinement at the component level (T-Persist-1(b)/(e), **both Cat A, Phase 12**). Core persistence chain: **all 5 parts (IFT, basin, threshold, interior gap, transport) Category A**. Merge dynamics, full coarsening cascade, and stochastic noise-driven formation birth/death rates remain open. These gaps are acknowledged, layer-classified, and constitute the primary agenda for Phase 13+.

...
- **Proved results registry:** Now **47 Category A**, **0 Category B**, **1 Category C**, **0 retracted**; total 48 claims (Phase 12: T-Persist-1(b) upgraded Cat B → A; Phase 13: T-Bind-Proj upgraded Cat B → A via explicit binary formula).
...
The proto-cohesion diagnostic vector $\mathbf{d} \in [0,1]^4$ unifies these four requirements into a graded formal assessment of formation quality, and the minimal energy principle — with mandatory volume constraint on the manifold $\Sigma_m$ — provides a variational characterization of formations. The theory has **47 fully proved theorems** (Category A), 0 with structural parameters (Category B), and 1 conditional result (Category C), with 0 retracted claims — totaling **48 formal claims, 97.9% fully proved** (Phase 13 upgrade: T-Bind-Proj Cat B → A via explicit binary mass-balance formula $\Phi(\tau; a_{\mathrm{cl}}, c)$ and volume-compatible threshold τ*(c) concept; Theorem 6.1 of R-BAR-BOUND.md retracted/corrected as Theorem 6.1'). Key results include non-trivial minimizer existence under a computable phase transition (T8-Core, T8-Full), gradient flow convergence (T14), stability advantage for non-idempotent closure (T3/T6), exact predicate-energy bridge (Sep = 1 - E_sep/m), closure residual bound for all τ (T-Bind-Proj), and the unified multi-formation persistence theorem (T-Persist-K-Unified) parametrized by the coupling measure $\Lambda_{\mathrm{coupling}}$. Phase 12-13 upgrades: T-Persist-1(b) basin containment Cat A via Theorem BC' + PSM; T-Bind-Proj general τ Cat A via binary formula + τ* concept. Core persistence chain (all 5 parts T-Persist-1(a-e)) now 100% Category A.
...
The theory's largest remaining gaps (Phase 13 completion: 97.9% Cat A, after T-Bind-Proj general τ Cat A upgrade) are: (1) general-graph FORMATION-BIRTH (proved for D₄-symmetric only, Cat A; general case Cat C); (2) near-bifurcation persistence (μ → 0, branch selection, center manifold reduction, Cat C). The unified regime parametrization ($\Lambda_{\mathrm{coupling}}$) provides a continuous bridge between well-separated, weakly-interacting, and strongly-interacting regimes with 100% experimental validation (exp46-47). T-Persist-K-Unified covers persistence below the merge bifurcation ($\Lambda < 1/(K{-}1)$); above this threshold, kinetic barrier-crossing dynamics dominate and the merge problem begins. Multi-formation temporal persistence is fully or conditionally resolved for: well-separated regime (T-Persist-K-Sep, **proved**), weakly-interacting regime (T-Persist-K-Weak, conditionally proved), parametric strong-coexistence regime below merge (T-Persist-K-Unified Corollary III, conditional), and basin containment/transport confinement at the component level (T-Persist-1(b)/(e), **both Cat A, Phase 12-13**). Core persistence chain: **all 5 parts (IFT, basin, threshold, interior gap, transport) Category A**. General-τ closure binding proven via binary mass-balance formula and volume-compatible threshold (T-Bind-Proj, **Cat A, Phase 13**). Merge dynamics, full coarsening cascade, and stochastic noise-driven formation birth/death rates remain open. These gaps are acknowledged, layer-classified, and constitute the primary agenda for Phase 14+.

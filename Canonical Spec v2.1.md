# Canonical Specification of Soft Cognitive Cohesion (v2.1)
...
- **Proved results registry:** Now **27 Category A** (was 24), **3 Category B**, **6 Category C**, **2 retracted**; total 36 claims.
...
*(Note: T-Bind-Proj/Full are effectively Category A for $\tau_{\mathrm{cl}} = 1/2$ since $\bar{r}_0 = O(n^{-1/d})$ is now analytically bounded. Retained in Category B for general $\tau$ where the bound is regime-specific. T8-Full, Predicate-Energy Bridge, and Deep Core 2b have been upgraded to Category A as of v2.1.)*
...
*Status:* **Proved with structural parameter** (Category B). *(Erratum 2026-04-02: Upgraded via Theorem BC' — directional basin containment eliminates the hard threshold NB: μ ≥ 4.1. The ellipsoidal basin radius $r_{\mathrm{eff}} = \sqrt{2\Delta_{\mathrm{bdy}}/(f_1^2\mu + (1-f_1^2)\mu_2)}$ is 2.5-4.3× larger than the isotropic bound $r_{\mathrm{iso}}$, extending Tier 1 persistence to any μ > 0 with quantitative gentleness condition $\varepsilon < r_{\mathrm{eff}}/(4+2/\mu)$. The soft-mode fraction $f_1 \leq \sqrt{n_{\mathrm{bdy}}/n}$ is a structural bound from boundary-mode dominance (Cat A). See BC-PRIME-THEOREM.md.)* *(Erratum 2026-03-31: basin radius analysis corrected — see below.)* Basin radius via energy sublevel set: $r_{\mathrm{basin}} = \sqrt{2\Delta_{\min}/\lambda_{\max}}$ where $\Delta_{\min} = \min(\Delta_{\mathrm{core}}, \Delta_{\mathrm{ext}}, \Delta_{\mathrm{bdy}})$. Core escape barrier: $\Delta_{\mathrm{core}} \geq 0.0441\beta$ ($\beta$-independent ratio $r_{\mathrm{core}} \geq 0.210$). Exterior escape barrier: $\Delta_{\mathrm{ext}} = O(\beta)$ per node (Proposition E1, proved). **Boundary escape barrier:** $\Delta_{\mathrm{bdy}}$ is formation-shape-dependent; $r_{\mathrm{bdy}} \geq 0.210$ away from shape bifurcation but can be small near bifurcation ($\mu \to 0$). Soft modes are boundary-dominated (~90% weight on boundary nodes, exp19/exp21-23); *(Erratum 2026-04-01: boundary-mode dominance now analytically proved — Proposition BMD in BASIN-ESCAPE-ANALYSIS.md §8. The Hessian diagonal at core sites is ≥ 4α + 0.92β while boundary/spinodal sites can be as low as 4αd_max − β, forcing the minimum eigenvector onto the boundary subspace with core fraction O(1/β).)* Barrier stability under gentle transition: $\Delta_s \geq \Delta_t - 2\varepsilon_1$ (Proposition 4). Basin containment holds when $\varepsilon_1 < \Delta_t/4$ and $2\varepsilon_2 + 2\varepsilon_1/\mu < r_{\mathrm{basin}}$ (Proposition 5). *(Erratum 2026-04-01: quantitative $\Delta_{\mathrm{bdy}}$ formula derived — Taylor normal form along soft mode gives $\Delta_{\mathrm{bdy}} = \mu/2 \cdot (t^*)^2 + L_3/6 \cdot (t^*)^3 + L_4/24 \cdot (t^*)^4$ where $t^*$ solves $\mu + L_3 t/2 + L_4 t^2/6 = 0$; verified $<1\%$ error. Empirical basins are 3–12$\times$ larger than sublevel-set estimate (exp24), confirming conservativeness of the sublevel bound.)* *(I13, PERSIST-MORSE-ANALYSIS.md, BASIN-ESCAPE-ANALYSIS.md.)*
...
*Status:* **Proved with structural parameter** (Category B). *(Erratum 2026-04-02: Upgraded via Theorem TC' — formation-conditioned transport confinement. The perturbative bound $\|\tilde{u} - \hat{u}_t\| \leq \sqrt{n}(C_1\varepsilon_1/\mu + \eta_{\text{core}})$ with $\eta_{\text{core}} = ne^{-\gamma\Delta_\varphi^2/\varepsilon_{\text{OT}}}$ tightens the uniform bound by factor $\sim n^{1/4}$ (formation-conditioned decomposition into core + boundary contributions). At natural parameters ($n=100$, $\varepsilon_1=0.01$, $\mu=1$), displacement $\approx 0.17 < r_{\text{basin}} \approx 0.2$. See TC-FORMATION-CONDITIONED.md.)* *(Erratum 2026-03-31: fixed-point existence upgraded from conditional to proved; fingerprint reduced to 3 components; contraction constants tightened.)*
...
*Status:* **Conditional** on (BC'-K) and (TC-K) analytical proofs. Sep corollary proved. 100% geometric-Lambda agreement in 69 experimental configurations (exp46-47). *(T-PERSIST-K-UNIFIED.md, PHASE-AB-SYNTHESIS.md.)*
...
The proto-cohesion diagnostic vector $\mathbf{d} \in [0,1]^4$ unifies these four requirements into a graded formal assessment of formation quality, and the minimal energy principle — with mandatory volume constraint on the manifold $\Sigma_m$ — provides a variational characterization of formations. The theory has **27 fully proved theorems** (Category A), 3 with structural parameters (Category B), and 6 conditional results (Category C), with 2 retracted claims — totaling **36 formal claims, 75% fully proved**. Key results include non-trivial minimizer existence under a computable phase transition (T8-Core, T8-Full), gradient flow convergence (T14), stability advantage for non-idempotent closure (T3/T6), exact predicate-energy bridge (Sep = 1 - E_sep/m), and the unified multi-formation persistence theorem (T-Persist-K-Unified) parametrized by the coupling measure $\Lambda_{\mathrm{coupling}}$.

The theory's largest remaining gaps are: near-bifurcation persistence ($\mu \to 0$, branch selection), strongly-interacting multi-formation merge (barrier crossing, MS1-MS4), formation birth/death (quantitative theory), and strong self-referential transport uniqueness. The unified regime parametrization ($\Lambda_{\mathrm{coupling}}$) now provides a continuous bridge between well-separated, weakly-interacting, and strongly-interacting regimes with 100% experimental validation (exp46-47). T-Persist-K-Unified covers persistence below the merge bifurcation ($\Lambda < 1/(K{-}1)$); above this threshold, the theory stops and the merge problem begins. Multi-formation temporal persistence is resolved for the well-separated regime (T-Persist-K-Sep, proved), weakly-interacting regime (T-Persist-K-Weak, conditionally proved), and strong-coexistence regime (T-Persist-K-Unified Corollary III-a, conditional). Formation birth/death and merge dynamics remain open. These gaps are acknowledged, layer-classified, and constitute the primary agenda for future development.

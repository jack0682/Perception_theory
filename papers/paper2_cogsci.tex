...
\item \textbf{Formation birth.} New formations arise through two proved mechanisms: parametric nucleation (supercritical pitchfork bifurcation when $\beta/\alpha$ crosses the Fiedler threshold) and topological splitting (graph topology change separates an existing formation). Volume-driven nucleation is ruled out by the isoperimetric energy ordering.

\item \textbf{Merge dynamics.} $K$-formation metastability is proved (local minimum, never saddle), and isoperimetric energy ordering establishes $K^*=1$ as the universal thermodynamic optimum. Energy barriers between multi-formation configurations are experimentally observed but the mountain pass proof of finite barriers has been retracted (endpoint domain error); a new proof strategy is needed.
...
\item The sole genuinely open theoretical problem is near-bifurcation persistence at exact bifurcation ($\mu = 0$); all other previously open items have been closed or conditionally closed. The non-bifurcation threshold has been tightened from $\mu > 6.3$ to $\mu > 3.4$ using a 3-component cohesion fingerprint, and the directional persistence extension widens the valid range by $2.5$--$4.3\times$, but $\mu = 0$ remains open. Stochastic stability experiments validate the enhanced metastability prediction ($p = 0.037$).
...
\item Multi-formation is fundamentally kinetic: $K^* = 1$ universally (proved, exp51), meaning $K > 1$ is never thermodynamically optimal. Multi-formation theory ($K > 1$) has been validated for $K = 2$ on grids up to $50 \times 50$, with T-Persist-K-Sep fully proved and T-Persist-K-Weak conditionally proved. Closure is a quality amplifier and stability enhancer (reduces d_min* by $\sim 30\%$, exp57), not an existence guarantor for K>1. Nucleation from random initial conditions almost always produces K=1 (>95\%, exp56); structured initialization is required for K>1 persistence. A merge barrier model $O(\beta^{0.89})$ explains metastable coexistence, with barriers far exceeding thermal noise scale (exp55: zero merging at $\sigma\leq 0.5$). Coarsening dynamics (predicted $\alpha < 1/2$) remain to be measured. The K-Strong saddle conjecture has been retracted (exp30: $K = 2$ configurations are local minima, not saddles). Coupled transport for strongly overlapping formations remains open.
\end{itemize}

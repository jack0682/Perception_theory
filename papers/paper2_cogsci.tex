...
\item \textbf{Multi-formation theory.} The $K$-field architecture supports multiple co-existing formations. Multi-formation persistence is proved in two regimes: T-Persist-K-Sep (well-separated, fully proved) and T-Persist-K-Weak (weakly interacting, conditionally proved). The K-Strong saddle conjecture---that strongly overlapping formations occupy saddle points---has been \emph{retracted} based on experiment (exp30): $K = 2$ configurations are always local minima, not saddles. Instead, an isoperimetric energy ordering is proved: a single formation of total volume $m$ always has lower energy than $K$ formations of total volume $m$, establishing that splitting is energetically unfavorable. The merge barrier is quantified as $O(\beta^{0.89})$, explaining why multiple formations persist as metastable states despite being energetically suboptimal. Formation birth has been formalized via three mechanisms: parametric nucleation (parameter change drives bifurcation), topological splitting (graph topology change separates a formation), and volume-driven nucleation (ruled out by the isoperimetric ordering). Scale has been verified to $50 \times 50$ grids with $K = 2$.
\end{enumerate}
...
\item Multi-formation theory ($K > 1$) has been validated for $K = 2$ on grids up to $50 \times 50$, with T-Persist-K-Sep fully proved and T-Persist-K-Weak conditionally proved. The K-Strong saddle conjecture has been retracted (exp30: $K = 2$ configurations are local minima, not saddles). A merge barrier model $O(\beta^{0.89})$ explains metastable coexistence. Formation birth is formalized via parametric nucleation and topological splitting (volume-driven nucleation ruled out by isoperimetric ordering). Coupled transport for strongly overlapping formations remains open.
\end{itemize}

...
We now establish four results governing how formations nucleate from homogeneous states and how multi-formation configurations merge. These results connect the static energy landscape to dynamic phenomena observed in experiments.
...
\label{thm:merge}
Let $\hat{u}^{(K)} \in \Sigma^K_M$ be a non-degenerate $K$-formation local minimizer of $\mathcal{E}_K$ (the $K$-field energy with repulsion). Then: (i)~$\hat{u}^{(K)}$ is a strict local minimum (not a saddle), with constrained Hessian bounded below by $\mu_K > 0$; (ii)~the global minimum on $\Sigma^K_M$ is the single-formation state $K=1$ (by isoperimetric ordering: one large core has less boundary energy per unit volume than $K$ small cores); (iii)~by the mountain pass theorem on the compact manifold $\Sigma^K_M$, there exists a generically non-degenerate index-1 transition state connecting $\hat{u}^{(K)}$ to $\hat{u}^{(K-1)}$; and (iv)~the Kramers escape rate gives the mean merge time
\begin{equation}
\label{eq:merge-rate}
\tau_{\mathrm{merge}} \sim \exp\!\bigl(\Delta E / \sigma^2\bigr), \quad \Delta E \leq \Theta(\beta),
\end{equation}
where $\Delta E$ is the energy barrier height and $\sigma^2$ is the noise intensity.
\end{theorem}

\begin{proof}[Proof sketch]
(i)~Non-degeneracy is the hypothesis; strict local minimality follows from second-order sufficiency on the constraint manifold. (ii)~For the boundary energy $\mathcal{E}_{\mathrm{bd}}$, the isoperimetric inequality on graphs gives $|\partial S| \geq h(G)|S|$ where $h(G)$ is the Cheeger constant. A single connected core of volume $V$ achieves boundary $O(V^{(d-1)/d})$, while $K$ cores of volume $V/K$ each have total boundary $K \cdot O((V/K)^{(d-1)/d}) = K^{1/d} \cdot O(V^{(d-1)/d})$, which is larger for $K > 1$. (iii)~The mountain pass theorem (Ambrosetti--Rabinowitz) applies to the $C^2$ energy $\mathcal{E}_K$ on the compact manifold $\Sigma^K_M$: two distinct local minima connected by a path guarantees a critical point of mountain pass type, which generically has Morse index~1. (iv)~The Kramers formula follows from large deviations theory for the stochastic gradient flow $du = -\nabla\mathcal{E}\,dt + \sigma\,dB_t$ restricted to $\Sigma^K_M$.
\end{proof}

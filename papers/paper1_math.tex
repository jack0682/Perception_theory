...
In the temporal domain, we prove fixed-point existence for self-referential transport (Schauder's theorem), establish a transport confinement bound for selection uniqueness ($C_{\mathrm{conf}} = O(\sigma\sqrt{\varepsilon_{\mathrm{OT}}\log n})$, independent of target field), prove isoperimetric energy ordering ($\mathcal{E}(u^*_{2m}) < 2\mathcal{E}(u^*_m)$ on connected graphs), and verify the full persistence chain computationally on grids up to $50 \times 50$.
...
\end{equation}
The three components are: raw cohesion, self-completion, and self-contrast. The resolvent diagonal $\mathbf{C}(x,x)$ is demoted from the canonical fingerprint: it contributes $<0.4\%$ of the fingerprint gap but has Jacobian norm $\sim$9300, making the cost matrix poorly conditioned. Every component depends only on $(u, \mathbf{N})$---no external features are required.
...
We identify five remaining open problems, ordered by mathematical importance.

"""
_r1_NQ221_no_simplex.py — NQ-221 Dynamic test with λ_bar=0

Phase 7 R1.1: same as Q1 but with λ_bar=0 (no simplex barrier). Tests whether
simplex barrier alone stabilizes the antisym Goldstone perturbation.
"""
...
def project_volume(u, m, clip=True):
    u = u + (m - u.sum()) / len(u)
    if clip:
        u = np.clip(u, 0, 1)
        for _ in range(5):
...
def main():
    ...
    LAMBDA_BAR = 0.0  # PHASE 7 KEY DIFFERENCE
    EPSILON = 0.05  # 10x larger than Q1
    ...
    # First find K=2 stationary point (with λ_bar=100 for proper convergence)
    ...
    print(f"Stationary K=2 (λ_bar=100): u1.max={fields_stat[0].max():.3f}, <u1,u2>={fields_stat[0]@fields_stat[1]:.3f}", flush=True)

    # Compute joint Hessian with λ_bar=0 (the test condition)
    print(f"\n=== Hessian with λ_bar={LAMBDA_BAR} ===", flush=True)
    H = joint_hessian(fields_stat, ec, lambda_rep, LAMBDA_BAR)
    ...
    print(f"Volume-projected H (λ_bar=0) lowest 6: {[round(float(e), 4) for e in eigvals[:6]]}", flush=True)

    # Pick lowest volume-preserving eigvec
    chosen_idx = None
    for i in range(20):
        v = eigvecs[:, i]
        s1 = abs(v[:n].sum())
    ...
    print(f"Selected eigvec {chosen_idx}: λ = {chosen_lam:.4f}, predicted τ = {1/abs(chosen_lam):.2f}", flush=True)

    # Perturb + ODE evolve with λ_bar=0
    ...
    fields_pert = [project_volume(u, c * n, clip=True) for u in fields_pert]

    print(f"Perturbation ε={EPSILON}, λ_bar={LAMBDA_BAR} for ODE evolution", flush=True)
    dt = 0.01; t_max = 50.0
    ...
    for step in range(n_steps):
        for j in range(2):
            ...
            fields[j] = project_volume(fields[j], masses[j], clip=True)
        if step % log_every == 0:
            ...
            log.append({
                ...
                'simplex_max_violation': float(np.max(fields[0] + fields[1] - 1)),
            })
    ...
    if mask.sum() > 5:
        ...
        rate = coeffs[0]
        print(f"Measured rate (λ_bar=0): {rate:.4f} (predicted |λ|={abs(chosen_lam):.4f}); ratio={rate/abs(chosen_lam):.3f}", flush=True)
    else:
        rate = None
        print(f"Insufficient data for fit (mask sum: {mask.sum()})", flush=True)

    # Check simplex violation (should be SUBSTANTIAL since λ_bar=0)
    max_viol = max(d['simplex_max_violation'] for d in log)
    print(f"Max simplex violation observed: {max_viol:.3f}", flush=True)
    ...
    out = {
        ...
        'eigvals_volproj_no_simplex': [float(e) for e in eigvals[:10]],
        'chosen_idx': chosen_idx,
        'chosen_lam': float(chosen_lam),
        'rate_measured': float(rate) if rate is not None else None,
        'max_simplex_violation': max_viol,
        'log': log,
    }
    out_path = os.path.join(os.path.dirname(__file__), 'results', 'r1_1_NQ221_no_simplex.json')

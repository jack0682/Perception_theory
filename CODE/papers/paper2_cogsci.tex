...
This simplified presentation uses crisp core sets for accessibility; the companion mathematical paper develops a soft transport formulation with quantitative bounds. Persist remains the least uniform of the four predicates: single-formation basin containment is now unconditional, several multi-formation persistence statements are proved only in explicit regimes, and near-bifurcation persistence at exact $\mu = 0$ remains open. The computational implementation includes both a core-overlap approximation and a transport-based \texttt{persist\_transport()} that implements the core-inheritance formula above using self-referential transport cost via cohesion fingerprints; in the weak transport regime, the self-referential fixed-point converges in 2--3 iterations.
...
\textbf{P9: Slower coarsening in self-referential phase separation.} In systems where the local order parameter affects its own energetic contribution, SCC predicts sub-diffusive coarsening: characteristic domain size evolves as $L(t) \sim t^\alpha$ with $\alpha < 1/2$ (slower than standard Allen-Cahn / Lifshitz-Slyozov-Wagner coarsening $\alpha = 1/2$). The slowdown arises from enhanced energy barriers (Theorem~4) that suppress formation merging. \emph{Caveat:} preliminary experiments find no coarsening under sustained noise up to $\sigma = 0.5$ (barrier O$(\beta) \approx 20 \gg$ noise scale), suggesting barriers may prevent coarsening until very long timescales or very small noise regimes.

\emph{Method:} Long-horizon gradient flow with $K_{\mathrm{initial}} \gg 1$ on large graphs; direct comparison of SCC vs.\ Allen-Cahn coarsening exponents. Alternatively, STEM time-series of spinodal decomposition with concentration-dependent interfacial properties. \emph{Testability:} \textbf{LOW-MEDIUM}---computational measurement is tractable; materials experiments require specialized conditions.
...
We implemented the SCC energy functional on finite grid graphs (sizes $5 \times 5$ to $20 \times 20$) using semi-implicit projected gradient descent with Barzilai-Borwein step sizes and multi-start initialization. The implementation comprises nine modules (${\sim}$2,900 lines of Python), verified against finite differences to tolerance $10^{-9}$ across 175 unit tests covering axiom satisfaction, energy decomposition, gradient accuracy, diagnostic properties, and temporal transport.
...
\textbf{Grid scaling.} Results are consistent across grid sizes from $5 \times 5$ to $50 \times 50$ ($n = 2{,}500$), with 100\% formation success rate at all scales. Deep core ratio increases from 0.68 ($10 \times 10$) to 0.91 ($50 \times 50$), and boundary-to-core ratio scales as $O(n^{-1/2})$ (measured slope $-0.499$, theory $-0.500$).
...
\item \textbf{The temporal component has substantial computational validation.} The Persist predicate is trivially satisfied in the static (single-time) experiments. The stochastic stability results (Langevin escape) validate the enhanced metastability prediction but do not test the full temporal transport chain. Transport fixed-point existence is now \emph{proved} (Schauder fixed-point theorem, valid for any $\varepsilon_{\mathrm{OT}} > 0$), not merely conjectured. The transport kernel uses a 3-component cohesion fingerprint (closure, distinction, co-belonging; resolvent demoted from formal universe to diagnostic role). A comprehensive warm-start chain experiment (exp27) achieves $5/5 \times 5/5 = 100\%$ PASS across all temporal scenarios, and a stress test (exp28) passes 84/100 configurations (all failures at $n < 64$ or $\beta < 20$, consistent with theory). Scale has been verified to $50 \times 50$ grids. Transport selection shows no multiplicity: a $\lambda_{\mathrm{tr}}$ sweep from 0.01 to 10 (exp29) finds a unique fixed point at every tested value. A transport confinement bound is proved: $C_{\mathrm{conf}} = O(\sigma \sqrt{\varepsilon_{\mathrm{OT}} \log n})$, independent of the target field $u_s$. Fingerprint-based transport concentration has been verified: core-to-core inheritance fractions exceed $99\%$ for well-formed formations, with deep core sites achieving $>99.99\%$ concentration. This supports the theory's central claim that temporal identity is structural inheritance---the transport kernel routes cohesive roles from core to core based on fingerprint similarity---rather than pointwise tracking via external labels.
...
\item \textbf{Multi-formation theory: kinetic framework.} Multi-formation is kinetically determined, not thermodynamically selected. Key results: (1)~\emph{K*=1 universally} (exp51): on every connected graph tested (homogeneous grids, stochastic block models, random geometric graphs, barbell graphs), a single formation has strictly lower energy than any $K > 1$ configuration of equal total volume, establishing the isoperimetric wall. (2)~\emph{Single-field coexistence impossible} (exp57): $K$ initial bumps on a single soft field always merge to $K=1$ under gradient flow, regardless of closure strength ($a_{\mathrm{cl}} = 0$ to $3.5$). However, closure demonstrably expands multi-formation stability: on a single field with well-separated formations (inter-formation distance $> 5$ nodes at $\beta=30$), SCC ($a_{\mathrm{cl}}=3.0$) maintains $K=4$ on a $10 \times 10$ grid while Allen-Cahn merges to $K=1$. This quantifies closure's role: reducing the critical distance $d_{\min}^*$ by $\sim 30\%$ (from $\approx 7$ to $\approx 5$ nodes), a multi-formation instantiation of the enhanced metastability theorem. (3)~\emph{K-field architecture is structurally necessary}: multiple independent fields $u^k$ with simplex constraint $\sum_k u^k(x) \leq 1$ prevent mass transfer between formations, enabling $K > 1$ persistence. (4)~\emph{Metastability parameters}: multi-formation coexistence remains empirically long-lived, and sustained noise up to $\sigma = 0.5$ produces zero coarsening on the tested grids; however, the relevant barrier surrogates are now best treated as branch-conditioned and protocol-tagged rather than as a universal exponent law. (5)~\emph{Nucleation}: random initialization converges to $K=1$ in $>95\%$ of trials; the spectral eigengap predicts nucleation sites but not final formation count (Pearson $r = 0.29$ with K-final). The $K$-field architecture supports multiple co-existing formations. Multi-formation persistence is proved in two regimes: T-Persist-K-Sep (well-separated, fully proved) and T-Persist-K-Weak (weakly interacting, conditionally proved). The K-Strong saddle conjecture has been retracted: $K = 2$ configurations are always local minima, not saddles. Scale has been verified to $50 \times 50$ grids with $K = 2$.
\end{enumerate}
...
\item \textbf{Transport fixed-point existence.} The self-referential transport fixed point exists for any $\varepsilon_{\mathrm{OT}} > 0$ (Schauder fixed-point theorem). This was previously conjectured; it is now proved.

\item \textbf{Transport confinement bound.} $C_{\mathrm{conf}} = O(\sigma \sqrt{\varepsilon_{\mathrm{OT}} \log n})$, independent of the target field $u_s$, bounding how far transport can displace cohesive mass.

\item \textbf{Isoperimetric energy ordering.} A single formation of total volume $m$ always has lower energy than $K$ formations of equal total volume. This establishes that formation splitting is energetically unfavorable.

\item \textbf{$K = 2$ local stability.} Merge-direction Hessian curvature is strictly positive at $K = 2$ critical points, confirming that multi-formation configurations are local minima (not saddle points).

\item \textbf{T-Bind upgrade.} The analytical bound $\bar{r}_0 = O(n^{-1/d})$ upgrades T-Bind to Category~A for the default threshold $\tau = 1/2$.

\item \textbf{Directional persistence extension.} Near-bifurcation persistence range is $2.5$--$4.3\times$ wider than the isotropic estimate, using basin geometry rather than worst-case radius.

\item \textbf{Formation birth.} New formations arise through two proved mechanisms: parametric nucleation at the spectral threshold $\beta/\alpha > 4\lambda_2/|W''(c)|$ and topological splitting (graph topology change separates an existing formation). In symmetry/simple-eigenvalue cases the local branch is supercritical; fully general supercritical branch selection on arbitrary degenerate eigenspaces remains open. Volume-driven nucleation is ruled out by the isoperimetric energy ordering.

\item \textbf{Merge dynamics.} $K$-formation metastability is proved (local minimum, never saddle), and isoperimetric energy ordering establishes $K^*=1$ as the universal thermodynamic optimum. Energy-barrier surrogates between multi-formation configurations are experimentally observed, but the branch-free mountain-pass proof of finite barriers has been retracted (endpoint domain error); current continuation-access evidence instead points to protocol-tagged, branch-conditioned merge and accessibility statements as the safer next target.

\item \textbf{Structured spectral bounds.} A $33\times$ improvement over the Weyl bound for multi-formation coexistence, exploiting formation-specific Hessian structure rather than generic eigenvalue estimates.

\item \textbf{Computational validation.} Formation existence and diagnostic vector properties are confirmed in 175 passing tests across grid sizes $5 \times 5$ to $50 \times 50$ ($n = 2{,}500$). A comprehensive verification experiment (exp44) tests 14 key predictions simultaneously, all passing.
\end{itemize}

\subsection{What Is Conjectured but Unproved}

Several items previously listed as conjectured have been proved since the initial submission: core depth from energy minimization (H2', deep core existence), transport fixed-point existence (Schauder, any $\varepsilon_{\mathrm{OT}} > 0$), the full phase transition (T8-Full, conditional on generic non-degeneracy, which holds generically by Sard's theorem), C3'' symmetrization (Sinkhorn--Lipschitz argument), and formation birth (pitchfork bifurcation and topological splitting). What remains genuinely conjectured:

\begin{itemize}
\item \textbf{Bifurcation branch selection at $\mu = 0$.} The supercritical pitchfork structure is confirmed, but the mechanism that selects among symmetry-broken branches at exact bifurcation remains unknown.

\item \textbf{Tight transport confinement constants.} The proved confinement bound $C_{\mathrm{conf}} = O(\sigma \sqrt{\varepsilon_{\mathrm{OT}} \log n})$ is valid but conservative: experiments show the bound is $25$--$10{,}000\times$ larger than observed confinement, suggesting the true constants are much tighter.

\item \textbf{Basin containment at exact bifurcation.} The non-bifurcation condition NB-1 shows basin collapse at $\mu = 0$. The directional persistence extension widens the valid range by $2.5$--$4.3\times$, but persistence at exact bifurcation ($\mu = 0$) remains open.
...
\item Near-bifurcation persistence at exact bifurcation ($\mu = 0$) remains a central theoretical gap: the non-bifurcation threshold has been tightened from $\mu > 6.3$ to $\mu > 3.4$ using a 3-component cohesion fingerprint, and the directional persistence extension widens the valid range by $2.5$--$4.3\times$, but $\mu = 0$ remains open. In addition, the latest branch-selection audits show that accessibility to low-energy branch families is protocol-dependent, so multi-formation merge and threshold claims now require explicit protocol tags rather than branch-free wording. Stochastic stability experiments still validate the enhanced metastability prediction ($p = 0.037$).
...
\item Multi-formation is fundamentally kinetic: $K^* = 1$ universally (proved, exp51), meaning $K > 1$ is never thermodynamically optimal. Multi-formation theory ($K > 1$) has been validated for $K = 2$ on grids up to $50 \times 50$, with T-Persist-K-Sep fully proved and T-Persist-K-Weak conditionally proved. Closure is a quality amplifier and stability enhancer (reduces $d_{\min}^*$ by $\sim 30\%$, exp57), not an existence guarantor for K>1. Nucleation from random initial conditions almost always produces K=1 (>95\%, exp56); structured initialization is required for K>1 persistence. What is newly clear is that K=2 branch selection and continuation-access are protocol-sensitive: some low-energy Type~B families are easy to preserve once seeded yet difficult to reach from raw search. Accordingly, branch-free merge-barrier scalars are no longer an honest summary of the current state, even though metastable coexistence remains empirically real. The K-Strong saddle conjecture has been retracted (exp30: $K = 2$ configurations are local minima, not saddles). Coupled transport for strongly overlapping formations remains open.
\end{itemize}

\subsection{Structural Limitations}

\begin{itemize}
\item \textbf{Diagnostic-energy asymmetry.} The forward direction (low energy implies high diagnostic scores) is proved: $\Bind \geq 1 - \sqrt{\mathcal{E}_{\mathrm{cl}}/n}$, and analogous bounds hold for Sep and Inside. However, the reverse direction does \emph{not} hold: high diagnostic scores do not guarantee low energy. This asymmetry is expected---the diagnostic vector decomposes formation quality, but multiple energy configurations can yield similar diagnostic profiles---yet it limits the diagnostic vector's role to assessment rather than optimization target. The $\bar{r}_0$ analytical bound ($O(n^{-1/d})$) upgrades T-Bind to Category~A for the default threshold $\tau = 1/2$, resolving the main structural parameter gap.
...
Several generalization directions strengthen the theory's mathematical foundations and broaden its applicability. (1)~A well-posed continuum limit exists, establishing that the finite-graph theory has a meaningful infinite-dimensional analogue. (2)~Stochastic dynamics (Langevin simulations) validate the enhanced metastability prediction and demonstrate that self-referential closure provides a relational restoring force absent in standard phase-field models. (3)~Multi-scale renormalization group analysis explains the topology-dependent value of self-referential terms: separation energy remains relevant under coarse-graining on community-structured graphs. (4)~The self-referential optimal transport formulation gives rise to a coupled PDE--OT system that is a novel mathematical object with potential connections to mean-field game theory. (5)~Formation birth theory formalizes three mechanisms by which new formations can appear: parametric nucleation (parameter change drives supercritical bifurcation), topological splitting (graph topology change separates an existing formation), and the ruling-out of volume-driven nucleation via the isoperimetric energy ordering. (6)~Protocol-tagged accessibility and barrier surrogates now provide the safer framework for multi-formation metastability: the evidence supports hard-to-enter / easy-to-keep low-energy families, but not a branch-free universal merge-barrier law. (7)~Coarsening exponent measurement: predicted $\alpha < 1/2$ (slower than Allen-Cahn) requires long-horizon simulations with $K_{\mathrm{initial}} \gg 1$; preliminary experiments find no coarsening under sustained noise (barriers $\gg$ noise scale), suggesting barriers suppress merging until very long times. (8)~Nucleation theory: the Fiedler eigenvector structure predicts nucleation sites, but the map from spectral eigengap to initial formation count $K_{\mathrm{nucleated}}$ is absent (eigengap uncorrelated with K-final, Pearson $r = 0.29$, exp56); connecting spectral structure to kinetic trajectories remains open. These directions are developed in the companion mathematical paper.
...
The companion mathematical paper establishes 35 fully proved theorems (Category~A), 4 structural results (Category~B), and 5 conditional results (Category~C), with 5 retracted claims, for an honest completion rate of 71\%. Multi-formation theory is proved in two regimes (well-separated and weakly interacting), with the isoperimetric energy ordering establishing that formation splitting is always energetically unfavorable.

SCC generates ten empirical predictions, at least four of which (P1, P2, P4, P5) are testable with standard psychophysical and EEG methods. Five predictions have received computational validation on graph-based domains: P1 (contraction rate confirmed), P3 (enhanced survival under noise, $p = 0.037$), P4 (path dependence confirmed in 96\% of trials), P5 (distinction precedes articulation in gradient flow dynamics), and P2 (partially confirmed). A comprehensive verification experiment (exp44) tests 14 key predictions simultaneously, all passing. The most immediately consequential for empirical testing is P5 (self-referential distinction precedes object recognition), which directly distinguishes SCC from predictive processing accounts.

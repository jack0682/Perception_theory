#!/usr/bin/env python3
"""Experiment 26: End-to-end T-Persist-1 dependency chain verification.

Traces ALL 5 parts of T-Persist-1 on a single formation across multiple
parameter regimes:
  (a) IFT continuity: ‖û_s - û_t‖ ≤ 2ε₁/μ
  (b) Gradient flow convergence from transported field to û_s
  (c) Deep core existence (Gap 6) — structural prerequisite
  (d) Exact threshold preservation: Core_t ⊆ Core_s
  (e) Transport core-to-core concentration

Reports WHERE the chain is tightest and which parameter regime is most fragile.
...
def bfs_depth(adj_list, core_set, start_nodes):
    """BFS from boundary inward to compute depth of each core node.

    Depth 0 = core nodes adjacent to non-core nodes (boundary of core).
    Depth k = core nodes whose shortest path to boundary (within core) is k.
    """
    n = len(adj_list)
    depth = np.full(n, -1, dtype=int)

    # Find core boundary: core nodes adjacent to at least one non-core node
    boundary = []
    for x in core_set:
...
def run_config(cfg):
    """Run full T-Persist chain for one configuration."""
    N, beta = cfg["N"], cfg["beta"]
    name = cfg["name"]
    n = N * N

    print(f"\n{'='*72}")
    print(f"  CONFIG: {name}  (n={n}, beta_bd={beta})")
    print(f"{'='*72}")

    results = {}

    # ---------------------------------------------------------------
    # Step 1: Find formation û_t
    # ---------------------------------------------------------------
    print("\n[1] Finding formation û_t ...")
    ...
    deep_core_set = set(deep_core_idx.tolist())

    # BFS depth from core boundary
    depth = bfs_depth(adj, core_t_set, core_t_idx)
    core2_idx = np.array([x for x in core_t_idx if depth[x] >= 2])

    # Interior gap: min_{x in Core²} (u_t(x) - theta_core)
    if len(core2_idx) > 0:
    ...
    print(f"    |DeepCore(θ={THETA_DEEP})| = {len(deep_core_idx)}")
    print(f"    Max core depth: {max_depth}")
    print(f"    Interior gap (min u - θ_core over Core²): {interior_gap:.6f}")
    ...
    if len(core2_idx) > 0:
        # Pick a deep core site (highest u) and an exterior site (lowest u)
    ...
    # ---------------------------------------------------------------
    # Step 4: ε-gentle perturbation + IFT continuity (part a)
    # ---------------------------------------------------------------
    print(f"\n[4] ε-gentle perturbation (ε₁={EPS_PERTURB}) ...")

    # Perturb alpha_bd by ε₁
    alpha_s = p_t.alpha_bd * (1.0 + EPS_PERTURB)
    p_s = ParameterRegistry(beta_bd=beta, alpha_bd=alpha_s)

    t0 = time.time()
    res_s = find_formation(g, p_s)
    dt4 = time.time() - t0
    u_s = res_s.u

    # IFT bound: ‖û_s - û_t‖ ≤ 2ε₁/μ
    # μ = smallest eigenvalue of Hessian at û_t (estimate from energy computer)
    field_diff = float(np.linalg.norm(u_s - u_t))
    field_diff_rel = field_diff / max(np.linalg.norm(u_t), 1e-12)

    # Estimate μ from Hessian spectral properties
    ec = EnergyComputer(g, p_t)
    ec.normalize_weights()
    # Use the gradient norm at the minimizer as a proxy for μ estimation
    grad_at_min = ec.gradient(u_t)
    grad_norm = float(np.linalg.norm(grad_at_min - np.mean(grad_at_min)))

    # For IFT: ε₁ is the parameter perturbation magnitude
    eps1 = abs(alpha_s - p_t.alpha_bd)
    # Heuristic μ estimate: use the energy curvature via finite differences
    # along the perturbation direction
    E_t, _ = ec.energy(u_t)
    delta_u = u_s - u_t
    delta_u_norm = np.linalg.norm(delta_u)
    if delta_u_norm > 1e-12:
        # Second-order estimate: E(u_s) ≈ E(u_t) + g·δu + μ/2 |δu|²
        ec_s = EnergyComputer(g, p_s)
        ec_s.normalize_weights()
        E_s, _ = ec_s.energy(u_s)
    ...
    part_a_pass = field_diff < ift_bound * 5  # generous factor for estimated μ

    print(f"    û_s converged: {res_s.converged}, time: {dt4:.2f}s")
    print(f"    ‖û_s - û_t‖₂ = {field_diff:.6f}")
    print(f"    ‖û_s - û_t‖₂/‖û_t‖ = {field_diff_rel:.6f}")
    print(f"    ε₁ (param perturbation) = {eps1:.6f}")
    print(f"    μ estimate = {mu_estimate:.6f}")
    ...
    # Core-to-core concentration for deep core sites
    core_s_idx = find_core(u_s, THETA_CORE)
    core_s_set = set(core_s_idx.tolist())
    ...
    if len(core_t_idx) > 0 and len(core_s_idx) > 0:
        ...
        part_e_pass = mean_conc > 0.5  # majority of mass goes core-to-core

        print(f"    |Core_t| = {len(core_t_idx)}, |Core_s| = {len(core_s_idx)}")
        print(f"    Core-to-core concentration (all core):")
        print(f"      min={min_conc:.4f}, mean={mean_conc:.4f}, "
              f"median={median_conc:.4f}")
        print(f"    Deep core (depth≥2) concentration:")
        print(f"      min={deep_min_conc:.4f}, mean={deep_mean_conc:.4f}")
    ...
    # Apply transport to get initial guess
    ...
    # Run gradient descent from transported field toward û_s
    t0 = time.time()
    res_flow = find_formation(g, p_s, u_init=u_init)
    dt6 = time.time() - t0
    u_flow = res_flow.u

    # Distance from gradient flow result to û_s
    dist_to_us = float(np.linalg.norm(u_flow - u_s))
    dist_to_us_rel = dist_to_us / max(np.linalg.norm(u_s), 1e-12)

    # Distance from transported init to û_s
    dist_init_to_us = float(np.linalg.norm(u_init - u_s))

    # Check if flow converged to same basin as û_s
    # Use energy comparison: if energies are close and fields are close
    ...
    print(f"    Transported field → û_s distance: {dist_init_to_us:.6f}")
    ...
    part_d_pass = frac_relaxed >= 0.9  # 90% of core preserved with relaxation
    ...
    # Method 1: Core overlap (diagnostics.py style)
    overlap = float(np.sum(np.minimum(u_t, u_s)) /
                    max(np.sum(u_t), np.sum(u_s)))

    # Method 2: Transport-based persist
    ...
    print(f"\n{'─'*50}")
    print(f"  CHAIN SUMMARY for {name}")
    print(f"{'─'*50}")
...
def print_summary_table(all_results):
    ...
    print(f"\n\n{'='*90}")
    print(f"  COMPREHENSIVE SUMMARY: T-Persist-1 Chain Verification")
    print(f"{'='*90}")
    ...
    print(f"\n{'Config':<16} {'‖Δu‖':>10} {'IFT bnd':>10} {'Core²':>6} "
          f"{'IntGap':>8} {'ConcMn':>8} {'FlowD':>10} {'Overlap':>8}")
    print(f"{'─'*16} {'─'*10} {'─'*10} {'─'*6} {'─'*8} {'─'*8} {'─'*10} {'─'*8}")
    ...
    # Fragility analysis
    print(f"\n{'─'*90}")
    print(f"  FRAGILITY ANALYSIS")
    print(f"{'─'*90}")

    # Find tightest margins
    tightest_ift = min(all_results,
                       key=lambda x: x[1]['ift_bound'] / max(x[1]['field_diff'], 1e-12))
    tightest_core = min(all_results, key=lambda x: x[1]['interior_gap'])
    tightest_conc = min(all_results, key=lambda x: x[1]['conc_mean'])
    tightest_flow = max(all_results, key=lambda x: x[1]['dist_flow_to_us_rel'])

    print(f"  Tightest IFT margin:     {tightest_ift[0]['name']} "
          f"(bound/diff = {tightest_ift[1]['ift_bound']/max(tightest_ift[1]['field_diff'],1e-12):.3f})")
    print(f"  Smallest interior gap:   {tightest_core[0]['name']} "
          f"(gap = {tightest_core[1]['interior_gap']:.6f})")
    print(f"  Weakest concentration:   {tightest_conc[0]['name']} "
          f"(mean = {tightest_conc[1]['conc_mean']:.4f})")
    print(f"  Worst flow convergence:  {tightest_flow[0]['name']} "
          f"(rel dist = {tightest_flow[1]['dist_flow_to_us_rel']:.6f})")

    # Overall verdict
    all_close = all(r['all_pass'] for _, r in all_results)
    n_configs_pass = sum(1 for _, r in all_results if r['all_pass'])

    print(f"\n  VERDICT: {n_configs_pass}/{len(all_results)} configs have complete chain closure")
    if all_close:
        print(f"  ★ T-Persist-1 chain VERIFIED across all parameter regimes")
    else:
        ...
        for name, parts in failing:
            ...
            print(f"  ✗ {name}: broken at {', '.join(broken)}")


def main():
    print("Experiment 26: End-to-end T-Persist-1 Chain Verification")
    print("=" * 60)
    print(f"Perturbation ε₁ = {EPS_PERTURB}")
    print(f"OT regularization ε_OT = {EPS_OT}")
    print(f"Core threshold θ = {THETA_CORE}")

...
\item Spectral threshold universality: the formation-birth threshold depends only on spectral data ($\lambda_2$), while fully general supercritical branch structure on arbitrary degenerate eigenspaces remains a structural gap.
\item Kinetic multi-formation paradigm: $K > 1$ formations coexist only metastably, with branch selection, accessibility, and barrier surrogates now understood to be regime-dependent and protocol-tagged rather than branch-free scalars.
\end{enumerate}
...
\item Multi-formation is fundamentally kinetic, not thermodynamic. The single-formation global minimum ($K^* = 1$, exp51) is the universal thermodynamic optimum on all connected graphs, established through the isoperimetric energy ordering. Multi-formation ($K > 1$) persists as metastable states, but the relevant barriers and selected branches are now understood to be branch-conditioned and protocol-sensitive rather than branch-free scalars. The merge theorem establishes: (a) $K$-formation metastability (local minimum, never saddle) and (b) isoperimetric energy ordering ($K^* = 1$ universally). Parts (c)--(e) (finite barrier via mountain pass, Kramers merge rate) are retracted due to an endpoint domain error ($\hat{u}^{(K-1)} \notin \Sigma^K_M$); the experimentally observed merge obstruction remains real but requires a new proof strategy. The critical inter-formation distance $d_{\min}^*$ is best treated as branch-conditioned rather than as a universal scalar formula. The $K$-field architecture (coupled soft cohesion fields with simplex constraint $\sum_k u^k(x) \leq 1$ and inter-formation repulsion) enforces $K > 1$ by construction; on a single field, $K > 1$ configurations collapse to $K=1$ under gradient flow. However, closure demonstrably expands the stability region on a single field: the critical inter-formation distance is reduced by roughly $30\%$ in the tested regime (exp57), enabling well-separated formations to coexist even without the K-field architecture. In the K-field formulation, temporal persistence is resolved in two regimes: (i)~well-separated formations persist independently (T-Persist-K-Sep, proved under per-formation T-Persist-1 hypotheses plus spectral-repulsion compatibility $\min_k \mu_k > (K{-}1)\lambda_{\mathrm{rep}}$); (ii)~weakly interacting formations (boundary overlap only) persist via joint IFT on the product manifold $\Sigma^K_M$ with Weyl spectral-gap control (T-Persist-K-Weak, conditionally proved). The strongly interacting regime remains open, and recent continuation-access evidence shows that initial conditions and search protocol matter as much as barrier height for which branch is actually observed.
...
Multi-formation is a kinetic phenomenon: $K^* = 1$ is the universal thermodynamic optimum on all connected graphs (isoperimetric ordering, proved), but $K > 1$ persists through metastability. Three open kinetic sub-problems remain: (i)~\emph{Nucleation:} the map from graph spectral structure (Fiedler eigenvector, eigengap) to initial formation count $K_{\mathrm{nucleated}}$. Current: random IC yields $K \approx 1$ for all graphs ($>95\%$, exp56); eigengap prediction is uncorrelated with outcome (Pearson $r = 0.29$). (ii)~\emph{Protocol-aware metastability:} quantifying how merge obstruction and branch selection depend jointly on separation, initialization, and search protocol. Current continuation-access evidence says some low-energy families are hard to enter but easy to preserve, so no branch-free barrier exponent is currently honest. (iii)~\emph{Coarsening:} characterizing the long-time dynamics $K(t)$ starting from $K_{\mathrm{initial}} > 1$. Theory predicts slower coarsening than Allen--Cahn ($\alpha < 1/2$, exp55 finds no coarsening under sustained noise up to $\sigma = 0.5$, consistent with barriers $\gg$ noise). The strongly interacting regime, merge-split bifurcation theory on $\Sigma^K_M$, and the interplay between spectral structure, protocol choice, and kinetic trajectories remain open.
\end{openproblem}
...
The framework now comprises 35 fully proved theorems (Category~A), 4 structural results (Category~B), 5 conditional results (Category~C), and 5 retracted claims. We have been transparent about limitations. The separation energy provides verified refinement but is complementary rather than dominant. Temporal persistence is no longer a monolithic open problem: single-formation basin containment and several transport components are closed, whereas multiple persistence statements remain regime-dependent and therefore conditional. Stochastic stability experiments confirm the enhanced metastability prediction: SCC formations survive noise $1.5\times$ more often than Allen--Cahn ($p = 0.037$). The diagnostic-energy relationship is one-directional: low energy guarantees high diagnostic scores, but the reverse does not hold. Parameters are structurally robust but not derived from empirical data. Destructive testing reveals that SCC requires relational heterogeneity: on complete graphs (uniform connectivity), the distinction operator cannot discriminate interior from exterior and formation fails entirely. On star graphs (extreme degree heterogeneity), formation quality degrades substantially. These are design boundaries, not bugs---the theory predicts that formation requires relational structure, and graphs lacking such structure correctly produce no formations.

The multi-formation extension now operates within a kinetic and protocol-sensitive framework: $K^* = 1$ is the universal thermodynamic global minimum (proved via isoperimetric ordering, exp51), but $K > 1$ states persist metastably only in regime-dependent windows. Well-separated and weakly interacting regimes have formal persistence theorems (T-Persist-K-Sep, T-Persist-K-Weak) with spectral-repulsion compatibility $\min_k \mu_k > (K{-}1)\lambda_{\mathrm{rep}}$, whereas branch selection and relaxed merge barriers are now understood to be protocol-tagged rather than branch-free scalars. Recent continuation-access evidence shows that some low-energy Type~B branch families are easy to preserve once seeded but difficult to access from raw multistart initialization, so future merge-barrier and threshold statements must specify branch family, search protocol, and path class explicitly. The most significant remaining open problems are therefore kinetic and search-sensitive: (1)~coarsening dynamics and exponent $\alpha$ (theory predicts $\alpha < 1/2$ vs.\ Allen--Cahn's $\alpha = 1/2$), (2)~nucleation theory connecting spectral eigengap to initial $K$, and (3)~protocol-aware merge and branch-selection theory on $\Sigma^K_M$. The self-referential optimal transport uniqueness problem remains the most mathematically significant single-formation open question.

...
This document is the canonical human-readable formal specification of the theory of Soft Cognitive Cohesion, version 2.0. It supersedes the original Canonical Spec (v1.0) and incorporates all mandatory changes from Iterations 1–5.
...
**Multi-formation architecture.** The contraction regime ($a_{\mathrm{cl}} < 4$) guarantees a unique closure fixed point, which effectively limits the theory to single-formation analysis. The distinction operator computes against $1 - u_t$ (the global complement), making it incoherent for multiple overlapping formations. Extending to multiple formations requires either leaving the contraction regime (losing A3), adopting $K$-field decomposition (multiple coupled fields), spectral decomposition of $\mathbf{C}_t$, or acknowledging bounded scope. This is the theory's most architecturally consequential open problem. The $K$-field architecture (I9) is the current binding decision, with `scc/multi.py` implementing the basic structure.
...
**T-Persist-K-Strong. Multi-Formation Dichotomy (Strongly-Interacting Regime).**
When formations have significant bulk overlap ($|O_{jk}| > \eta \cdot \min(|\text{Core}_j|, |\text{Core}_k|)$), temporal evolution follows a dichotomy controlled by the joint spectral gap $\mu_{\text{joint}} = \lambda_{\min}(H_{\text{joint}})$ on $\Sigma^K_M$, where $H_{\text{joint}}$ has diagonal blocks $H_k$ and off-diagonal blocks $\lambda_{\text{rep}} I$: (1) if $\mu_{\text{joint}} > 0$ (compatible overlap, requires (SR): $\min_k \mu_k > (K-1)\lambda_{\text{rep}}$), the IFT applies and formations persist with $K_s = K_t$; (2) if $\mu_{\text{joint}} \leq 0$ (incompatible overlap, (SR) violated), the $K$-formation critical point becomes a saddle under perturbation, the gradient flow descends to a $(K{-}1)$-formation minimizer, and post-hoc separation destroys formation identity. Experimental evidence (exp14 E2) confirms the bimodal pattern: $\lambda_{\text{rep}} = 0.5$ with overlap 36 gives Persist 0.96/0.98 (compatible), while $\lambda_{\text{rep}} = 0.1$ with overlap 33 gives Persist 0.47/0.42 (incompatible merge).
*Status:* **Conjectured** with experimental support. Full proof requires Morse-theoretic bifurcation analysis on $\Sigma^K_M$. *(docs/03-31/repair/MULTI-TEMPORAL-THEORY.md §6.)*

### Bridging

**Temporal proto-cohesion (Persist).** *(Erratum 2026-03-31: Major advances — 3 of 4 open conditions closed.)* T-Persist-Full (§13) synthesizes all components into a unified theorem under hypotheses (WR', PS, ND, NB, H2', H3, GT). T-Persist-1(a,c) proved; T-Persist-1(b) conditionally proved via basin radius (boundary-mode dominated; $r \geq 0.210$ away from bifurcation, can be smaller near bifurcation — BASIN-ESCAPE-ANALYSIS.md); T-Persist-1(d) conditionally proved under H2'+H3 (H2' now proved: deep core existence via isoperimetric, CORE-DEPTH-ISOPERIMETRIC.md; $C_2 \leq 2.875$ sharp); T-Persist-1(e) fixed-point existence proved via Schauder (entropic regularization $\Rightarrow$ continuity), concentration with tightened 3-component fingerprint ($\|\partial\varphi/\partial u\|_{\text{op}} \leq 1.43$, $\mu_0 \geq 3.4$). Remaining genuinely open: (i) near-bifurcation persistence ($\mu \to 0$ at shape transitions, basin radius $\to 0$); (ii) strong-regime fixed-point selection/uniqueness near $\hat{u}_t$.
...
*Status:* **Proved** for the tangential component $\|r_T\|_2$, with the mean residual $\bar{r}_0$ as an explicit structural parameter.

*Honest assessment of the mean residual.* The mean component $\bar{r}_0 = |\sum_i(\mathrm{Cl}(\hat{u})_i - \hat{u}_i)|/n$ is not controlled by the KKT argument. It reflects a structural property of the closure operator: $\mathrm{Cl}$ does not preserve the volume constraint ($\sum_i \mathrm{Cl}(u)_i \neq m$ in general), so $\bar{r}_0$ measures the per-site mass mismatch. The projected KKT eliminates the Lagrange multiplier $\nu$ and controls $r_T$, but cannot reach $\bar{r}_0$ without circularity (substituting the $\nu$ bound back into the residual equation produces a vacuous inequality). Empirically, $\bar{r}_0 < 0.02$ at all tested formations across parameter regimes, because well-formed formations are approximately binary (interior sites: $\mathrm{Cl}(\hat{u})_i \approx \hat{u}_i \approx 1$; exterior sites: $\mathrm{Cl}(\hat{u})_i \approx \hat{u}_i \approx 0$). Making this rigorous requires a quantitative binary-approximation result at finite parameters, which is a separate open problem.

**Predicate-Energy Bridge (Forward Direction).** Small $\mathcal{E}_{\mathrm{cl}}(\hat{u})$ implies high Bind; small $\mathcal{E}_{\mathrm{sep}}(\hat{u})$ implies high Sep. The directional implication is proved; quantitative thresholds are not.

**Predicate-Energy Bridge (Exact Equalities).** $\mathsf{Sep} = 1 - \mathcal{E}_{\mathrm{sep}}/m$ (exact, for $u$-weighted Sep). $\mathsf{Bind} \geq 1 - \sqrt{\mathcal{E}_{\mathrm{cl}}/n}$ (Cauchy-Schwarz). Both proved for the current predicate definitions.

**T8-Full. Non-Trivial Minimizer under Full Energy.**
Adding $\lambda_{\mathrm{cl}}\,\mathcal{E}_{\mathrm{cl}} + \lambda_{\mathrm{sep}}\,\mathcal{E}_{\mathrm{sep}}$ to $\mathcal{E}_{\mathrm{bd}}$ does not destroy the non-uniform minimizer for $\lambda_{\mathrm{cl}}, \lambda_{\mathrm{sep}}$ small relative to $\beta$. Conditional on non-degeneracy of the $\mathcal{E}_{\mathrm{bd}}$ minimizer's constrained Hessian, the IFT gives a smooth family of perturbed minimizers $\hat{u}(\lambda_{\mathrm{cl}}, \lambda_{\mathrm{sep}})$ with $\|\hat{u} - \hat{u}_0\| = O(\lambda_{\mathrm{cl}} + \lambda_{\mathrm{sep}})$. The threshold is $\delta = \mu_0 / (2\max(\|H_{\mathrm{cl}}\|, \|H_{\mathrm{sep}}\|))$ where $\mu_0$ is the minimum Hessian eigenvalue of $\mathcal{E}_{\mathrm{bd}}$.
*Proof:* IFT on the bordered KKT system $F(u, \nu, \lambda) = (\nabla\mathcal{E}(u;\lambda) - \nu\mathbf{1},\; \mathbf{1}^T u - m) = 0$. The bordered Hessian is invertible iff $\mu_0 > 0$. Differentiating at $\lambda = 0$ gives the perturbation bound with $C = \max(\|\nabla\mathcal{E}_{\mathrm{cl}}(\hat{u}_0)\|, \|\nabla\mathcal{E}_{\mathrm{sep}}(\hat{u}_0)\|)$. *(GAP-CLOSURES.md §G9.)*
*Caveat:* Non-degeneracy ($\mu_0 > 0$) is generically true (Sard's theorem) and computationally confirmed at default parameters, but not proved for specific graphs.
...
*Status:* **Conditionally proved** (basin containment under GT+ND+NB). *(Erratum 2026-03-31: basin radius analysis corrected — see below.)* Basin radius via energy sublevel set: $r_{\mathrm{basin}} = \sqrt{2\Delta_{\min}/\lambda_{\max}}$ where $\Delta_{\min} = \min(\Delta_{\mathrm{core}}, \Delta_{\mathrm{ext}}, \Delta_{\mathrm{bdy}})$. Core escape barrier: $\Delta_{\mathrm{core}} \geq 0.0441\beta$ ($\beta$-independent ratio $r_{\mathrm{core}} \geq 0.210$). Exterior escape barrier: $\Delta_{\mathrm{ext}} = O(\beta)$ per node (Proposition E1, proved). **Boundary escape barrier:** $\Delta_{\mathrm{bdy}}$ is formation-shape-dependent; $r_{\mathrm{bdy}} \geq 0.210$ away from shape bifurcation but can be small near bifurcation ($\mu \to 0$). Soft modes are boundary-dominated (~90% weight on boundary nodes, exp19/exp21-23). Barrier stability under gentle transition: $\Delta_s \geq \Delta_t - 2\varepsilon_1$ (Proposition 4). Basin containment holds when $\varepsilon_1 < \Delta_t/4$ and $2\varepsilon_2 + 2\varepsilon_1/\mu < r_{\mathrm{basin}}$ (Proposition 5). *(I13, PERSIST-MORSE-ANALYSIS.md, BASIN-ESCAPE-ANALYSIS.md.)*
...
*Status:* **Conditionally proved under H2'+H3.** *(Erratum 2026-03-31: H2 upgraded to H2' — deep core existence proved.)* The Interior Gap Lower Bound establishes that for core sites at graph distance $\delta(x) \geq 2$ from $\partial\text{Core}$: $\hat{u}(x) - \theta_{\text{core}} \geq (1 - \theta_{\text{core}}) - 2\exp(-c_0\,\delta(x)) - C_{\text{op}} R/\beta$, where $c_0 = \text{arccosh}(1 + \kappa^2/d_{\min})$, $\kappa = \sqrt{\beta/(2\alpha)}$, $C_{\text{op}} \leq 2(1 + a_{\text{cl}}/4) \cdot R$ with $R = (\lambda_{\text{cl}} + \lambda_{\text{sep}})/\lambda_{\text{bd}}$, giving $C_2 \leq 2.875$ at default parameters. Requires: **(H2') Deep core existence:** $\text{Core}^2(\hat{u}) \neq \emptyset$ — proved for $|\text{Core}| \geq 25$ and $\beta/\alpha \gg 1$ via $\Gamma$-convergence isoperimetric analysis (Theorem 1, CORE-DEPTH-ISOPERIMETRIC.md). Deep Core Dominance: $|\text{Core}^2|/|\text{Core}| \geq 1 - 4/\sqrt{m}$ (Theorem 2). (H3) $\beta > 11\alpha$ (for $d_{\min} = 4$). *(I13, PERSIST-PDE-ANALYSIS.md, CORE-DEPTH-ISOPERIMETRIC.md.)*
...
*Status:* **Conditionally proved** (fixed-point existence proved via Schauder; concentration conditional on interior gap). *(Erratum 2026-03-31: fixed-point existence upgraded from conditional to proved; fingerprint reduced to 3 components; contraction constants tightened.)*

*Fixed-point existence (Schauder):* For any $\varepsilon_{\text{OT}} > 0$, the self-referential transport map $T : \Sigma_m \to \Sigma_m$ has a fixed point. Proof: entropic regularization makes the inner OT problem strictly convex $\Rightarrow$ $M^*(c)$ is unique and continuous in $c$ (Berge's maximum theorem) $\Rightarrow$ $T$ is continuous on compact convex $\Sigma_m$ $\Rightarrow$ Schauder applies. Uniqueness holds in the weak regime (Banach contraction); selection near $\hat{u}_t$ guaranteed when (WR) holds. *(TRANSPORT-CONCENTRATION-STRENGTHENED.md §4.)*

*Canonical fingerprint:* $\varphi = (u, \mathrm{Cl}(u), D(x;1\!-\!u)) \in [0,1]^3$ (3 components; resolvent $C(x,x)$ demoted to optional diagnostic — contributes $<0.4\%$ of fingerprint gap but $\|\partial C/\partial u\| \approx 9300$ dominates Jacobian). With 3-component fingerprint: $\|\partial\varphi/\partial u\|_{\text{op}} \leq 2.83$ (analytical), measured $\approx 1.43$ at formations.
...
*Status:* **Conditionally proved** under (H1-K), (WI), (SR), (NB-K), plus per-formation T-Persist-1 conditions. Full statement in §12.
...
- **(NB)** Non-bifurcation: $\mu_{\mathcal{F}} \geq \mu_0 > 0$ with $\mu_0 \gtrsim 3.4$ (away from shape transitions; tightened from 6.3 with 3-component fingerprint)
...
*Status:* **Conditional** on (NB) and energy barrier $\Delta_t > 0$. (H2') now proved; fixed-point existence now proved; (WR) relaxed to selection condition. Remaining open: near-bifurcation persistence (NB failure, where $r_{\mathrm{basin}} \to 0$); strong-regime uniqueness/selection.
...
**CN6. $K$ Must Be Emergent.** The relational kernel $K_t$ must ultimately be derived from the data or domain structure, not imposed externally. In the canonical theory it is a given; making it emergent is an implementation-layer concern.
...
The proto-cohesion diagnostic vector $\mathbf{d} \in [0,1]^4$ unifies these four requirements into a graded formal assessment of formation quality, and the minimal energy principle — with mandatory volume constraint on the manifold $\Sigma_m$ — provides a variational characterization of formations. The theory has 12 proved theorems, including non-trivial minimizer existence under a computable phase transition (T8-Core), gradient flow convergence (T14), and a proved stability advantage for non-idempotent closure (T3/T6).

The theory's largest remaining gaps are: strong-regime self-referential transport (existence and uniqueness when $\lambda_{\mathrm{tr}}$ is large), core depth $\delta_{\min} \geq 2$ from energy minimization alone, near-bifurcation persistence, and strongly-interacting multi-formation temporal evolution (dichotomy theorem). Basin containment for T-Persist-1(b) has been conditionally closed via the basin radius theorem ($r \geq 0.210$, $\beta$-independent) and barrier stability analysis; T-Persist-Full (§13) synthesizes all six original gaps into a unified conditional theorem under explicit hypotheses (WR, PS, ND, NB, H2, GT). The weak regime of self-referential transport is implemented and numerically verified, with all components of T-Persist-1 now proved or conditionally proved. Multi-formation temporal persistence is resolved for the well-separated regime (T-Persist-K-Sep, proved) and weakly-interacting regime (T-Persist-K-Weak, conditionally proved); the strongly-interacting regime and formation birth/death remain open. These gaps are acknowledged, layer-classified, and constitute the primary agenda for future development.

The theory now stands as a mathematically structured ontology of pre-objective cohesion — with genuine theorems, honest gap accounting, and a clear distinction between what is proved, what is provisional, and what remains open.

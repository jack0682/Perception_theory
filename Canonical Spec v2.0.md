...
*Status:* **Conditionally proved under H2'+H3.** *(Erratum 2026-03-31: H2 upgraded to H2' — deep core existence proved.)* The Interior Gap Lower Bound establishes that for core sites at graph distance $\delta(x) \geq 2$ from $\partial\text{Core}$: $\hat{u}(x) - \theta_{\text{core}} \geq (1 - \theta_{\text{core}}) - 2\exp(-c_0\,\delta(x)) - C_{\text{op}} R/\beta$, where $c_0 = \text{arccosh}(1 + \kappa^2/d_{\min})$, $\kappa = \sqrt{\beta/(2\alpha)}$, $C_{\text{op}} \leq 2(1 + a_{\text{cl}}/4) \cdot R$ with $R = (\lambda_{\text{cl}} + \lambda_{\text{sep}})/\lambda_{\text{bd}}$, giving $C_2 \leq 2.875$ at default parameters. Requires: **(H2') Deep core existence:** $\text{Core}^2(\hat{u}) \neq \emptyset$ — proved for $|\text{Core}| \geq 25$ and $\beta \geq 58\alpha$ via $\Gamma$-convergence isoperimetric analysis + exponential saturation bootstrap (Theorem 1, CORE-DEPTH-ISOPERIMETRIC.md). *(Erratum 2026-04-01: Step 4 Γ→finite-β transfer now rigorously proved via Markov inequality + Euler-Lagrange bootstrap, replacing hand-wave.)*  Deep Core Dominance: unconditionally $|\text{Core}^2| = |\text{Core}| - |\partial_V\text{Core}|$ (Theorem 2a, identity); conditionally $|\text{Core}^2|/|\text{Core}| \geq 1 - 4C/\sqrt{m}$ under iso\_ratio $\leq C$ (Theorem 2b). (H3) $\beta > 11\alpha$ (for $d_{\min} = 4$). *(I13, PERSIST-PDE-ANALYSIS.md, CORE-DEPTH-ISOPERIMETRIC.md.)*
...
- **(NB)** Non-bifurcation: $\mu_{\mathcal{F}} \geq \mu_0 > 0$ with $\mu_0 \gtrsim 4.1$ (formation-conditional bound; conservative $\mu_0 \gtrsim 6.7$; tightened from original 6.3 with 3-component fingerprint). Near bifurcation ($\mu < \mu_{\mathrm{bif}}$): basin collapse (Theorem NB-1, NEAR-BIFURCATION-LOCAL-THEORY.md §8) with $\Delta_{\mathrm{bdy}} = O(\mu^3)$; deep-core remnant persistence survives (Theorem NB-2)
...
*Status:* **Conditional** on (NB) and energy barrier $\Delta_t > 0$. (H2') now proved; fixed-point existence now proved (Schauder via finite-time flow); (WR) relaxed to selection condition. Near-bifurcation: formalized as three-tier persistence ladder (Theorems NB-1/NB-2): full persistence above $\mu_{\mathrm{bif}}$, deep-core remnant below, transition at $\mu \to 0$. Empirical basin 3–12$\times$ larger than sublevel estimate (exp24). End-to-end chain verified (exp26): parts (a)(c)(e) pass universally, (b)(d) require basin identity (warm-start, not multi-start). Remaining open: strong-regime uniqueness/selection; quantitative $\Delta_{\mathrm{bdy}}$ as function of formation geometry (Taylor normal form derived, BASIN-ESCAPE-ANALYSIS.md §9).

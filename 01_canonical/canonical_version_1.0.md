...
released: 2026-04-01
status: accepted
description: Baseline canonical specification with 12 Category A theorems from iterations 1-2
---

# Canonical Specification of Soft Cognitive Cohesion (v1.0)
...
*Status:* **Proved** (fully, for $\tau_{\mathrm{cl}} = 1/2$). *(Erratum 2026-04-01: The mean residual $\bar{r}_0$ is now analytically bounded. Theorem 6.1 of R-BAR-BOUND.md proves $\bar{r}_0 = O(|\partial\mathrm{Core}|/n) = O(n^{-1/d})$ in the sharp-interface regime via KKT contraction analysis + mass-balance cancellation. On 2D grids: $\bar{r}_0 = O(1/\sqrt{n})$. Scale verification (exp42, grids 10-30): boundary ratio slope = -0.499 (theory: -0.500). T-Bind upgraded from Category B to Category A for default parameters ($\tau = 1/2$).)*

...
> $$\boxed{\;\bigl\lVert \Pi_T^{\mathrm{free}} R_{\mathrm{cl}}(u^*) \Pi_T^{\mathrm{free}}\bigr \rVert_{\ell^2} \;\leq\; C_R \cdot \delta(u^*) \cdot \lVert r \rVert_2 \cdot \sqrt{\rho_{\mathrm{bd-band}}(u^*) \cdot n}\;} \tag{1.3}$$
> with explicit constant $C_R = 2\lambda_{\mathrm{cl}} \cdot a_{\mathrm{cl}}^2 \cdot \lVert M \rVert^2$ ($\lVert M \rVert_{D\to D} \leq 1$).
>
...
**핵심 도구:** `exp_hmorse_broadness_full_spectrum.py` 의 $\beta \in \{10, 20, 30, 50, 100\}$ × 격자 크기 3 = 15 configurations 에서 $\lVert R_{\mathrm{cl}} \rVert$ 직접 측정 → $\beta$ 의 함수로 fitting → 예측 (1.4) 의 *경험적* 형식 검증.
...
이 값들이 (3.10) 의 예측치 $7.4 \times 10^{-3}$ 와 *정량적으로 일치 안함* — bound 가 매우 conservative. **그러나** 핵심 비교는 $\mu_{\min} \geq c_{\mathrm{HML}}^{\mathrm{Cat A}} = 2\lambda_{\mathrm{cl}}(1-a_{\mathrm{cl}}/4)^2 (d_{\min}/d_{\max}) + \alpha\lambda_2 - 2\beta\rho_{\mathrm{bd-band}} - \lVert R_{\mathrm{cl}} \rVert$. 위 bound 가 *부등식의 우변에서 빼는 항* → $c_{\mathrm{HML}}$ 가 음수가 되지 않아야 함. 
...
(4.1) 의 RHS 가 $\lVert V \rVert = \lVert R_{\mathrm{cl}} \rVert$ — *우리가 Approach A 에서 bound 한 quantity 와 같다*. 따라서 Approach B 는 Approach A 의 결과를 *재포장* 할 뿐.

### §4.2 왜 부차적인가 — 정량 비교

| 측면 | Approach A | Approach B |
|---|---|---|
| 추정 대상 | $\lVert \Pi_T R_{\mathrm{cl}} \Pi_T \rVert$ (operator norm) | $\mu_{\min}$ shift (scalar) |
| 핵심 단계 | $\sigma''$ pointwise bound + Lemma L2/L3 | $\lVert V \rVert$ 추정 (Approach A 와 동일) |
| Cat 가능성 | **A** (해석적) | A *if* $\lVert V \rVert$ 추정이 explicit | 
...
| Grid | β | $\lVert r \rVert_2$ | $\mu_{\min}$ | pred (1.1, lcl) | ratio |
|---:|---:|---:|---:|---:|---:|
...
**Setup:** $\beta = 5 < \beta_{\mathrm{crit}}^{(2)}$ (canonical $\beta_{\mathrm{crit}}^{(1)} \approx 4\lambda_2 / \lvert W''(c) \rvert \approx 4 \cdot 0.038 / 0.94 \approx 0.16$ on 15×15 grid — *주의: very small for the lattice case*). T8-supercritical 조건 위반.
...
| (S3) Free-coord $\lVert w \rVert_\infty$ bound on $w \in \mathrm{Bd}_{\mathrm{spin}}$ | Cat C (no explicit bound) | OP-HMORSE-LOCAL-A-S3 |
...
2. **$\lVert M \rVert$ 의 conservative 추정** ($\leq 1$ — 실제로는 $\ll 1$ degree-weighted).
3. **Boundary-band 의 $\lVert w \rVert_\infty^2 \leq 1$** 가 *상한* — 실제 $w$ 가 *균등 분포* 시 $\lVert w \rVert_\infty^2 \approx 1/n$.
...
- (S3) Boundary-band $\lVert w \rVert_\infty$ explicit → OP-HMORSE-LOCAL-A-1.
...
- (C2′) Active-set non-empty: 15/15 PASS 의 모든 minimizer 에서 $\lvert A^* \rvert > 0$.
...
T8-supercritical 의 *non-empty*: $\beta/\alpha \in [10, 100]$ all > $4\lambda_2/\lvert W''(c) \rvert \approx 0.16$ (15×15 grid). 따라서 본 정리의 조건이 *명시적으로* canonical 수치 실험에서 실현.
...
| $\varepsilon_{\mathrm{Cl}}$ | Closure 잔여 $\lVert r \rVert_\infty$ | §3.1 Lemma L1 |
...
- §9.1 OP-HMORSE-LOCAL-A-1: $\lVert w \rVert_\infty$ explicit.

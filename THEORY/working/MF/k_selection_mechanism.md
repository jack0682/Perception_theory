...
**Working refs:** `K_status_commitment.md` (OAT-1, Commitment 16 origin); `foundational_bridges_2026.md` §8 (Bridge B-7 AC analog); `pre_objective_K_field_tension.md` (OAT-6); `multi_formation_sigma.md` (D-6a static framework, $\mathrm{Aut}(G)_{u^*}$ scaffolding); `sigma_multi_trajectory.md` §2 (K-jump definition); `stereo_observation_framework.md` §7 ($\mathcal{P}$-conditional partition function $Z_K(\mathcal{P})$ and free energy $F(K;\mathcal{P})$ extending candidate (a)+(b) to observation-conditioned setting, W6 D2).

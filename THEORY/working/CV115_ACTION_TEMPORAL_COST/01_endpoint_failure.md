...
$$c^\mathrm{end}_{t\to r}(x,z) = \lVert z - x \rVert^2$$
...
> Squared endpoint cost $c^\mathrm{end}(x,z) = \lVert z-x \rVert^2$는 일반적으로
>
...
$$c^\mathrm{end}_{t\to r}(0,2) = \lvert 2-0 \rvert^2 = 4$$

우변 (중간점 $y=1$을 통한 합성):

$$c^\mathrm{end}_{t\to s}(0,1) + c^\mathrm{end}_{s\to r}(1,2) = \lvert 1-0 \rvert^2 + \lvert 2-1 \rvert^2 = 1 + 1 = 2$$
...
$$\min_y\bigl[\lVert y-x \rVert^2+\lVert z-y \rVert^2\bigr] = 2\left\lVert \frac{z-x}{2}\right \rVert^2 = \frac{\lVert z-x \rVert^2}{2}$$

따라서:

$$c^\mathrm{end}(x,z) = \lVert z-x \rVert^2, \qquad \min_y[\ldots] = \frac{\lVert z-x \rVert^2}{2}$$

차이 $= \lVert z-x \rVert^2/2 \neq 0$ (일반적). 등호는 $x=z$일 때만 성립.
...
$$\frac{\lVert y^*-x \rVert^2}{s-t}
= \frac{\left(\dfrac{s-t}{r-t}\right)^2\lVert z-x \rVert^2}{s-t}
= \frac{s-t}{(r-t)^2}\lVert z-x \rVert^2$$

$$\frac{\lVert z-y^* \rVert^2}{r-s}
= \frac{\left(\dfrac{r-s}{r-t}\right)^2\lVert z-x \rVert^2}{r-s}
= \frac{r-s}{(r-t)^2}\lVert z-x \rVert^2$$

합산:

$$\frac{s-t}{(r-t)^2}\lVert z-x \rVert^2 + \frac{r-s}{(r-t)^2}\lVert z-x \rVert^2
= \frac{(s-t)+(r-s)}{(r-t)^2}\lVert z-x \rVert^2
= \frac{\lVert z-x \rVert^2}{r-t} \quad \blacksquare$$

**해석**:

이 결과가 보여주는 것:

1. **$1/\Delta t$ normalization이 핵심이다.** raw $\lVert z-x \rVert^2$는 합성 불가이지만, $\lVert z-x \rVert^2/(r-t)$는 등속 경로에서 정합.

2. **연속 시간 analogy**: $\int_t^r \lVert \dot\xi(\tau) \rVert^2\,d\tau$의 등속 경로 최솟값이 $\lVert z-x \rVert^2/(r-t)$.

3. **local action의 $d^2/\Delta t$ 항 설계 근거**: normalization 없이는 합성 시 오차 발생.

**단, 이 정합은 등속 경로에서만 성립한다.** 임의 경로에서는

$$\frac{\lVert z-x \rVert^2}{r-t} \;\leq\; \min_y\!\left[\frac{\lVert y-x \rVert^2}{s-t} + \frac{\lVert z-y \rVert^2}{r-s}\right]$$
...
| 비교 | 결과 | 의미 |
|---|---|---|
| $c^\mathrm{end}(x,z) = \lVert z-x \rVert^2$ | $\min_y[\ldots] = \lVert z-x \rVert^2/2 \neq \lVert z-x \rVert^2$ | 합성 불가 |
| $c^\mathrm{norm}(x,z) = \lVert z-x \rVert^2/(r-t)$ | 등속 중간점에서 additive | 부분적 정합 |

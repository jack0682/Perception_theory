...
**(ii) Hessian split.** *(Corrected 2026-04-27 evening per `91_critical_review.md` §3.)* By R22 normal-form analysis on the Fiedler doublet (cubic + quartic equivariants), the post-bifurcation minimizer's Hessian on $V_2 = \mathrm{span}(\phi_{(1, 0)}, \phi_{(0, 1)})$ has two leading-order eigenvalues:
...
where $A_1, A_2$ are the cubic-equivariant coefficients in the $D_4$ normal form $F(x, y; \beta) = \beta(x^2 + y^2) + A_1 (x^2 + y^2)^2 + A_2 x^2 y^2$. **On $D_4$ free-BC: $A_2/A_1 = 4$** (R22 §3.3 Cat A), so $\mu_1 = 4|W''(c)|\epsilon = \mu_0$ — both eigenvalues equal at leading order. The two modes are degenerate but **distinguished by irrep label**, not by eigenvalue magnitude. (Discrete $D_4 \to \mathbb{Z}_2$ symmetry breaking does NOT produce a continuous-symmetry Goldstone; the transverse mode is fully lifted to the same $\epsilon$-scale eigenvalue, distinguished only by sign-irrep transformation under $s_y$.) Higher modes stay at $\mu_k(c\mathbf{1}) + O(\sqrt{\epsilon})$.
...
**Theory status (post-W5 Day 6, 2026-05-02 CV-1.5.2): 46 Category A, 5 Category B, 5 Category C, 5 Retracted (61 claims, 75% fully proved).** Category B items: T-Bind-Proj (τ=1/2 sub-case status differs from canonical/theorem_status — see audit note in `THEORY/CHANGELOG.md`), T-Persist-K-Sep, T-Persist-K-Unified, T-σ-Theorem-4 (Cat A → Cat B retroactive at CV-1.5.1 per Critic verdict + NQ-187 numerical refutation, Cat A re-promotion deferred to CV-1.7+ post-γ/β/α path audit), T-σ-Multi-1 (Cat B target, Goldstone-pair instability). Category C items: T-Bind-Full (general τ), T-Persist-1(a/d), T-Persist-Full, T-Persist-K-Weak, plus V5b-F (Cat C new finding 2026-04-26) and the Cat C riders inside T-σ-Lemma-2 (v) Courant upper bound + (vi) $G_u$-orbit divisibility. Retracted: Thm 3.3 general τ, T-Merge (c), T-Merge (d), T-Merge (e) (merge path manifold error), K-Saddle Conjecture.

...
- **SPLIT** ($C_i^t \to \{C_{j_1}^s, C_{j_2}^s\}$): split direction $v_1 = \arg\min_{\lVert v \rVert=1} \langle v, H_i^t v\rangle$ (lowest Hessian eigenvector, Goldstone mode) Cat B; $\sigma_{\mathrm{standard}}$ Cat C.
...
- **Hypothesis package (P0)–(P11)**:
  ...
  - P5 background suppression $\lVert U \rVert_{\infty,X_{\mathrm{bg}}}\le\ell_{\min}-\rho_{\mathrm{bg}}$ (on $U$, not just on $R_{\mathrm{inact}}$);
...
| **T-σ-Theorem-4** | σ at First Pitchfork on $D_4$ Free-BC Grid (Leading Order, **continuum-limit claim**) | Cat A at CV-1.5; **Cat A → Cat B retroactive at CV-1.5.1 (2026-04-29)** per Critic 7-agent verdict (Errata Round 1 Morse-index inconsistency at merge time). **Continuum vs discrete grid caveat added 2026-05-04 W6 NQ-187 audit** (canonical.md §13 T-σ-Theorem-4 entry now contains explicit continuum-vs-discrete note): canonical statement (ii) — leading-order degeneracy $\mu_0 = \mu_1 = 4\lVert W''(c) \rVert\epsilon$ from $A_2/A_1 = 4$ — is a continuum-limit prediction (R22 §3.3 Lebesgue integral on unit square) **not realized on finite discrete grids** $L \le 16$. NQ-187 numerical (`logs/daily/2026-04-30/11_nq187_scaling_test_results.md`, script `CODE/scripts/test_sigma_theorem4_scaling.py`, $L \in \{4, 8, 16\}$, $\epsilon \in \{0.001..0.1\}$, analytic sparse $\Sigma_m$-Hessian + shift-invert Lanczos): measured $\mu_0/\epsilon \approx 1$, $\mu_1/\epsilon \approx 2$, ratio $\mu_1/\mu_0 \approx 2$ (not $1$ degeneracy) and exponent $p \approx 1.03$ (not $p = 2$ predicted, not $p = 3/2$ alternative). Three reconciliation hypotheses (α continuum extrapolation $L \to \infty$, β R22 derivation re-audit, γ $\Sigma_m$-Hessian convention) under audit. Cat A re-promotion deferred to CV-1.7+ post-γ/β/α path closure. | B (was A in $\epsilon$-small regime at CV-1.5) | C-0716 | P-0716 | (theoretical via T-Birth-Parametric + R22 axis-aligned, continuum-limit; numerical NQ-187 on discrete grid) | $D_4 \to \mathbb{Z}_2$ symmetry breaking; continuum prediction Mode 0 trivial irrep / Mode 1 sign irrep both with eigenvalue $4\lVert W''(c) \rVert\epsilon$ (degenerate at continuum leading order); on discrete $L \le 16$ grid measured $\mu_1/\mu_0 \approx 2$ and effective coefficient $\approx 1$ (not $4$). $\mathcal{F}$ tie-break NQ-143/NQ-184. Statement now read as **continuum-limit claim** until γ/β/α audit closes. |
...
| ID | Statement | Status | Cat | Conditions |
|---|---|---|---|---|
| **L-CLOSURE-LIFT** | $\lVert J_\mathrm{Cl} \rVert_{D\to D} \leq a_\mathrm{cl}/4 < 1$; $(I - J_\mathrm{Cl})^\top D (I - J_\mathrm{Cl}) \succeq (1 - a_\mathrm{cl}/4)^2 D$ uniformly | Cat A (CV-1.16, 2026-05-14) | A | $G$ connected; A3 ($a_\mathrm{cl} < 4$); $u^* \in [0,1]^n$ |
...
| **L-BOUNDARY-MODE-EXCLUSION** | At interior single-formation T8-supercritical minimizers, $\lVert v_\mathrm{min} \rVert_{\partial X}^2/\lVert v_\mathrm{min} \rVert^2 \leq 1/2 + O(\alpha/\beta)$ generically | Cat C (CV-1.16) | C | D-HMORSE-LOCAL (C1)(C2′)(C3)(C4); non-empty $\partial X$; SKETCH-level perturbation theory |
...
| **OP-HMORSE-EXCLUSION-VOLUME** | Boundary mode exclusion volume | Low | DOWNGRADED (CV-1.16): operationally resolved by $\Pi_T = I - (1/n) \mathbf{1} \mathbf{1}^\top$ producing single ~0 eigenvalue numerically. Formalization deferred. |
| **OP-HMORSE-ACTION-INTERACT** | H-MORSE × action-cost framework interaction | Low | OPEN (NEW CV-1.16): cross-package interaction questions between H-MORSE-Local (Layer 3 regularity) and action-cost composition (CV-1.15 Cat B). Non-blocking; investigated as opportunity arises. |
| **OP-PAI-001** | Formal definition of interpretation gap $\Delta_{\text{interp}}$ | High | OPEN (NEW 2026-05-21 PAI PIVOT): what mathematical object measures discrepancy between perceptual individuation $\mathcal{I}_{\text{perception}}(F)$ and action interpretation $\mathcal{I}_{\text{action}}(F)$? Candidates: structural distance on partitions; Hausdorff between segmentation maps; mutual information loss; categorical natural transformation defect; affordance-preservation distortion. No solution attempted. |
| **OP-PAI-002** | Action interpretation map $\mathcal{A}(u)$ | High | OPEN (NEW 2026-05-21 PAI PIVOT): how to define $\mathcal{A}(u)$ for a *single action class*? Action is plural — manipulation, navigation, attention, inspection, communication, repair, etc. Commitment to one class is a user decision. Action is NOT reducible to motor output. Includes affordance, controllability, manipulability, navigability, collision relevance, task relevance, temporal intervention, policy-conditioned meaning. |
| **OP-PAI-003** | Interpretation invariance criterion | High | OPEN (NEW 2026-05-21 PAI PIVOT): three candidate forms — (a) equivariance $\mathcal{P}(g \cdot x) = g \cdot \mathcal{P}(x)$ under $G_{\text{action}}$; (b) commutativity $\mathcal{A} \circ \mathcal{P} = \mathcal{P} \circ \mathcal{A}'$; (c) low-distortion $\Delta_{\text{interp}}(F) \leq \epsilon$. Choosing among them = this OP. |
| **OP-PAI-004** | Diagnostic vector extension | Medium | OPEN (NEW 2026-05-21 PAI PIVOT): should SCC's $d = (\text{Bind, Sep, Inside, Persist})$ be extended with action-readiness or invariance component, e.g., $d_{PA} = (\text{Bind, Sep, Inside, Persist, Act/Inv}_{PA})$? Existing 4 components are NOT modified; this is a candidate extension only. |
| **OP-PAI-005** | Tokenization/embedding critique formalization | Medium | OPEN (NEW 2026-05-21 PAI PIVOT): how to distinguish meaningful action-preserving decomposition from arbitrary computational tokenization? Constraint: must NOT silently revive 8 retractions (EW universality / Model A / $t_\times$ / $D_f$ / H-int / closure RG / $D_f = 11/8$ / $k(k+1)/2-1$). |
| **OP-PAI-006** | Formation-to-affordance bridge | Medium | OPEN (NEW 2026-05-21 PAI PIVOT): under what conditions does a cohesive formation generate affordances without a separate symbolic reinterpretation layer? Bridge to Gibsonian ecology / active inference is candidate, not committed. |
...
**W7 changes (2026-05-10 ~ 2026-05-14):**
- *CV-1.13 (W7-CV1.13, 2026-05-10):* T-Temporal-Identity full Cat A; OP-0011 RESOLVED (steps 1–3 closed).
- *CV-1.15 (W7-Day5 morning, 2026-05-14):* Action-Based Temporal Cost Package +8A +2B; OP-0012-SINK new sub-label OPEN (cost-level blocker closed under action redef; scaling-gap blocker remains).
- *CV-1.16 (W7-Day5 evening, 2026-05-14):* H-MORSE-Local Closure Package +1A +2B +1C; **OP-HMORSE-BROADNESS CLOSED Cat A**; 6 new OP-HMORSE-* registered (LOCAL-A / SADDLE / SBM / GENERIC-PATH / EXCLUSION-VOLUME / ACTION-INTERACT). H-MORSE row: OPEN → **PARTIALLY CLOSED** (in hypothesis_tree.md).
- *결정 C (W7-Day5, 2026-05-15):* 6 stage 사용자 통찰 검토 (z_t / S_0 / K_read), V=0 + archive 패턴 6/6 → 결정 C. canonical 0 edits. 새 OP 등록 없음. canonical 의 명시적 non-claim: z_t / S_0 / K_read reformulation 은 *시행 안 함*.
...
- **Scope restriction (cross-cite, hygiene CV-1.16+1):** L-ACTION-DELTA-EFF-ZERO and L-SOFT-ACTION-DELTA-EFF-ZERO apply *only to the action direct-cost redefinition* ($c^{\mathrm{direct}} := c^{\mathrm{act}}$), **not to endpoint cost or Sinkhorn-derived costs**. Silent application of these lemmas to the endpoint-cost path is unsound (cf. canonical.md §13 Cat A L-ACTION-DELTA-EFF-ZERO body; L-ENDPOINT-NONSEMI Cat A counterexample).
...
- **L-δ_eff-SINK** (Cat C target): quantitative bound on $\delta_{\mathrm{eff}}^{\mathrm{sink}} := \lVert c_{\mathrm{direct}}(u_t, u_r) - c^{\mathrm{eff}}(M^{\mathrm{sink}}_{t\to s}, M^{\mathrm{sink}}_{s\to r}) \rVert_\infty$ in terms of marginals, $\varepsilon_{\mathrm{OT}}$, structural separation $\Delta_{\mathrm{sep}}$.
- **L-Eff-Sinkhorn** (Cat C target): quantitative bound on $\lVert M^{\mathrm{sink}}(\mathbf{K}_{t\to r}) - M^{\mathrm{sink}}_{s\to r} \cdot M^{\mathrm{sink}}_{t\to s} \rVert_\infty$ in terms of $\lVert b_1 \odot a_2 - c \cdot \mathbf{1} \rVert$.

*Path to Cat B (proposed for CV-1.17+):* L-δ_eff-SINK + L-Eff-Sinkhorn both reach Cat C → under stable-K + margin + small-sink-gap → conditional Cat B theorem T-CC-StableK-Sinkhorn.

*Adjacent candidate (not registered to avoid OP-0022 collision with existing "Continuous-Time Limit" OP):* continuous-time action limit (Γ-convergence of discrete-time $\mathcal{A}_{i:k}$ to a continuous action functional on path space). Deferred CV-1.17+. **Naming note:** existing OP-0022 already denotes "Continuous-Time Limit" (theorem_status.md line 843); this candidate would be a refinement within OP-0022 rather than a new OP — naming to be settled CV-1.17+.

**Sub-case C — OP-0012-Kjump (K-jump general, Cat C):** depends on OP-0008 (σ-Inherit, MERGE/SPLIT) and OP-0021 ($T_*$ registration; note: this references the H-T* "T_* normalization" usage in hypothesis_tree.md HT-3.7, not the "Stochastic Dynamics" OP-0021 row at line 837 of this file — pre-existing OP-0021 dual-naming inconsistency flagged, to be reconciled CV-1.17+). Unchanged from CV-1.13.

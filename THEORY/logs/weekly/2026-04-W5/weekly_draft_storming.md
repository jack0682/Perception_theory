...
- **Tool A4 quantitative audit V1**: REJECTED (scope; PHR comparison framework based on incorrect simplex constraint assumption — SCC has no $\sum_j u^j \leq 1$ per site, only per-field mass $\lVert u^j \rVert_1 = m_j$). Tool A4 PARTIAL FAIL status maintained.

...
**Interaction with canonical.** No interaction with existing theorems. Could *replace* $\Phi_{\mathrm{res}}$ definition with a sharpness-parameterized family: "$\Phi_{\mathrm{res}}^s := \{\phi:\lVert \phi-\phi_{\mathrm{hard}} \rVert_{L^\infty\text{ off edge band}}\le 1/s\}$", which is cleaner axiomatically.

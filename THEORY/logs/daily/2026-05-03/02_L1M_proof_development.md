...
For Type-B bars, $b_i\in X_{\mathrm{bg}}$ where $X_{\mathrm{bg}}$ is the background region (off all $N_j^r$). On $X_{\mathrm{bg}}$ the field $U$ coincides with the inactive residual: $U\vert _{X_{\mathrm{bg}}}=R_{\mathrm{inact}}\vert _{X_{\mathrm{bg}}}$. By P10, $\lVert R_{\mathrm{inact}} \rVert_\infty\le\ell_{\min}-\rho_{\mathrm{res}}$. Hence the birth height $b_i=U(b_i)\le\ell_{\min}-\rho_{\mathrm{res}}$, and by P0 the death is $d_i=0$:
$$

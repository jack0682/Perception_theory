...
*Conditions.* L-HMORSE-DECOMP conditions; additionally **(T8-supercritical)** $\beta/\alpha > 4\lambda_2/|W''(c)|$ (canonical T8-Core Cat A); $u^*$ is the symmetry-broken interior minimizer with phase-separated support.
...
$$\|v_{\min}|_{\partial X}\|^2 / \|v_{\min}\|^2 \;\geq\; 1 - O(\alpha/\beta)$$
...
*Consequence for D-HMORSE-LOCAL (C5).* (C5) requires $\|v_{\min}|_{\partial X}\|^2 / \|v_{\min}\|^2 \leq 1/2$ — i.e., *exclude* configurations where the minimum eigenmode is boundary-localized. This excludes the genuine "boundary Goldstone" family from H-MORSE-Local scope.
...
- (C5) Compute $v_{\min}$; need $\|v_{\min}|_{\partial X}\|^2 / \|v_{\min}\|^2 \leq 1/2$.

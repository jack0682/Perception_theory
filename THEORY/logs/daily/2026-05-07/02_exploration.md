...
4. **Cost function.** Working file §4.1 uses $c(x,y) = \lVert \varphi(x) - \varphi(y) \rVert^2 + \sigma_\mathrm{sp}^{-2} \lVert x - y \rVert^2$. Canonical T-Persist-1(e) machinery uses essentially the same cost up to scaling. We commit to one explicit cost form and check that it satisfies the conditions of T-Persist-1(e) (TC1–TC3).
...
**Core idea.** Treat $R_{t\to s}$ as the discrete analogue of the *bottleneck-stable* matching between persistence diagrams $\mathrm{Dgm}(u_t)$ and $\mathrm{Dgm}(u_s)$. Stability theorems (Cohen-Steiner–Edelsbrunner–Harer) give: bottleneck distance between diagrams is $\leq \lVert u_t - u_s \rVert_\infty$. Lift the bottleneck matching to a component-level bijection.

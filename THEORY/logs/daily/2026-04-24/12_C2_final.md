...
>
> **(iii) Quantitative gap (Cat A via Lemma 4)**. The non-criticality residual $\lVert \pi_{\mathbf{1}^\perp}\nabla\mathcal{E}(u^*_\text{disk}) \rVert^2$ on the unit circle of $(\lambda_\text{cl}, \lambda_\text{sep})$ has minimum $\mu_\text{min}(M) > 0$, with $M = \begin{pmatrix} \lVert g_\text{cl} \rVert^2 & \langle g_\text{cl}, g_\text{sep}\rangle \\ \langle g_\text{cl}, g_\text{sep}\rangle & \lVert g_\text{sep} \rVert^2 \end{pmatrix}$ positive definite. Phase 2 L=12: $\mu_\text{min} \approx 1.78$.
>

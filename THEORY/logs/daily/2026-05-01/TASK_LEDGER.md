...
6. **γ-path execution at L=4 small grid** (16 sites): symbolic computation under both Convention I (centered) and Convention II (Lagrange) projection conventions; identify which (if either) reproduces canonical $\mu_0 = 4\vert W''(c)\vert \epsilon$ formula.

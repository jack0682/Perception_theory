...
> **Lemma L1.** $H_\mathrm{merged}(u^*) = H_0 + V$, $\|V\|_\mathrm{op} \leq \lambda_\mathrm{rep} c e^{-c_0 d_\mathrm{inter}}$. 이때 perturbative regime $\|V\| < \min_a |\lambda_a^{(0)} - \lambda_b^{(0)}|$ 에서 post-merger eigenvalue $\lambda_a$ 의 Kato expansion 은 $O(\varepsilon^2)$ 까지 *deterministic* 형태로 산출됨.

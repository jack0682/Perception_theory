...
Let $G = (V, E)$ be a finite graph, $n = \lvert V \rvert$, and fix total cohesion mass $M > 0$. Set per-slot mass $m = M / K_{\mathrm{field}}$ (equal distribution; the equal-mass case is canonical in I9).
...
For each subset $P \subseteq \{1, \ldots, K\}$ with $\lvert P \rvert \geq 2$, define the *P-diagonal stratum*:
...
**Boundary/corner strata:** For $\lvert P \rvert = K$ (all slots equal), dim $= n-1$, codim $= (K-1)(n-1)$.

**Stratification of $\widetilde{\widetilde\Sigma}^K_M$:**
$$\widetilde{\widetilde\Sigma}^K_M = \pi(\Sigma^K_{M,\mathrm{free}}) \sqcup \bigsqcup_{P \subseteq \{1,\ldots,K\},\, \lvert P \rvert \geq 2} \pi(D_P^\circ)$$
...
| Symmetric stratum $D_P$ (equal slots in $P$, $\lvert P \rvert \geq 2$) | $(K - \lvert P \rvert + 1)(n-1)$ |
